...
def includeme(config):
    ...
    config.add_route('samplePage',
                     '/samplePage/{samplename}',
                     factory=sample_page_factory)
...
def page_factory(request):
    pagename = request.matchdict['pagename']
    page = request.dbsession.query(project).filter_by(
        project_ID=pagename).first()
    if page is None:
        raise HTTPNotFound
    return PageResource(page)
...
def spectrometer_page_factory(request):
    ...
    page = request.dbsession.query(experiment).filter_by(
        experiment_ID=pagename).first()
    if page is None:
        raise HTTPNotFound
    return PageResource(page)
...
def molecule_page_factory(request):
    ...
    page = request.dbsession.query(molecule).filter_by(
        molecule_ID=pagename).first()
    if page is None:
        raise HTTPNotFound
    return PageResource(page)
