...
def experimentForm(request):
    ...
    if 'submit' in request.POST:
        ...
        try:
            ...
            request.dbsession.add(page)
            next_url = request.route_url('experimentPage',
...
@view_config(route_name='experimentForm2',
             renderer='../templates/experimentForm2.jinja2')
def experimentForm2(request):
    ...
    if 'submit' in request.POST:
        ...
        controls = request.POST.items()
        project_ID = request.matchdict[
            'project_ID']  #this is the only different field - get the related project ID
        controls = request.POST.items()
        pstruct = peppercorn.parse(controls)
...
@view_config(route_name='experimentPage',
             renderer='../templates/experimentPage.jinja2')
...
all the values, it also contains buttons for adding samples and experiments."""
    ...
    searchdb = request.dbsession.query(experiment).filter_by(
        experiment_ID=search).all()
    dic = {}
    for u in searchdb:
        new = u.__dict__
        dic.update(new)
    search2 = request.dbsession.query(experimental_conditions).filter_by(
        experiment_ID=search).all()
    dic2 = {}
    for u in search2:
        new = u.__dict__
        dic2.update(new)
    search3 = request.dbsession.query(data_aquisition).filter_by(
        experiment_ID=search).all()
    dic3 = {}
    for u in search3:
        new = u.__dict__
        dic3.update(new)
    search4 = request.dbsession.query(
        spectra.spectra_ID).filter_by(experiment_ID=search).all()
    spec = {}
    #dic of related spectra
    for u in search4:
        num = u[0]
        search = request.dbsession.query(
            post_processing_and_deposited_spectra.final_published_spectrum
        ).filter_by(spectra_ID=num).first()
        spec[(num)] = search[0]
    ...
    for k, v in spec.items():
        ...
        dic4[k] = colory
        jcamp_dict = JCAMP_reader(
            os.path.join('ftirdb', 'data', 'infrared_spectra', spec[k]))
    ...
    if 'form.submitted' in request.params:
        if request.params['form.submitted'] == 'Add spectrometer':
            ...
            next_url = request.route_url('spectrometerForm',
