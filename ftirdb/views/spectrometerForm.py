...
light_source_choice = (('', '- Select -'), ('globar', 'globar'),
                       ('laser', 'laser'), ('synchrotron', 'synchrotron'),
                       ('other', 'other'), ('', ''))
beam_splitter_choice = (('', '- Select -'), ('KBr', 'KBr'), ('Mylar', 'Mylar'),
                        ('other', 'other'), ('', ''))
detector_choice = (('', '- Select -'), ('DTGS', 'DTGS'), ('MCT Broad band',
                                                          'MCT Broad band'),
                   ('MCT narrow band', 'MCT narrow band'), ('other',
                                                            'other'), ('', ''))
optics_choice = (('', '- Select -'), ('vacuum', 'vacuum'), ('purged', 'purged'),
                 ('dry', 'dry'), ('atmospheric',
                                  'atmospheric'), ('', ''), ('other', 'other'))
recording_choice = (('', '- Select -'), ('fourier transform',
                                         'fourier transform'), ('dispersive',
                                                                'dispersive'),
                    ('tunable laser', 'tunable laser'), ('', ''), ('other',
                                                                   'other'))
mode_choice = (('', '- Select -'), ('transmission', 'transmission'),
               ('ATR', 'ATR'), ('transflectance', 'transflectance'),
               ('diffuse reflection',
                'diffuse reflection'), ('other', 'other'), ('', ''))
window_choice = (('', '- Select -'), ('CaF2', 'CaF2'), ('BaF2', 'BaF2'),
                 ('ZnSe', 'ZnSe'), ('ZnS', 'ZnS'), ('CdTe', 'CdTe'),
                 ('KBr', 'KBr'), ('KRS-5', 'KRS-5'), ('other', 'other'), ('',
                                                                          ''))
prism_choice = (('', '- Select -'), ('Diamond', 'Diamond'), ('Ge', 'Ge'),
                ('Si', 'Si'), ('KRS-5', 'KRS-5'), ('ZnS', 'Zns'),
                ('ZnSe', 'ZnSe'), ('', ''), ('other', 'other'))


@view_config(route_name='spectrometerForm',
             renderer='../templates/spectrometerForm.jinja2')
def spectrometerForm(request):
    ...
    if 'submit' in request.POST:
        ...
        try:
            ...
            page = spectrometer(experiment_ID=exp,
                                optics=optics,
                                beamsplitter=beamsplitter,
                                type_of_recording=type_of_recording,
                                detector__type=detector,
                                light_source=light_source,
                                mode_of_recording=mode_of_recording,
                                **ok)
            request.dbsession.add(page)
            # add the rest to database using **kwargs
            spectrometer_id = request.dbsession.query(spectrometer).order_by(
                spectrometer.spectrometer_ID.desc()).first()
            spectrometer_id = spectrometer_id.spectrometer_ID
            pok = pstruct['atrSchema']
            page = atr(**pok,
                       prism_material=prism,
                       spectrometer_ID=spectrometer_id)
            request.dbsession.add(page)

            naok = pstruct['not_atrSchema']
            page = not_atr(**naok,
                           sample_window_material=window,
                           spectrometer_ID=spectrometer_id)
            request.dbsession.add(page)
            ...
            experiment_id = request.dbsession.query(spectrometer).filter_by(
                optics=optics).first()
            spec_id = experiment_id.spectrometer_ID

            next_url = request.route_url('spectrometerPage',
...
@view_config(route_name='spectrometerForm2',
             renderer='../templates/spectrometerForm2.jinja2')
...
@view_config(route_name='spectrometerPage',
             renderer='../templates/spectrometerPage.jinja2')
def spectrometerPage(request):
    ...
    if 'form.submitted' in request.params:
        ...
        searchdb = request.dbsession.query(spectrometer).filter_by(
            spectrometer_ID=search).all()
        dic = {}
        for u in searchdb:
            new = u.__dict__
            dic.update(new)
        searchothers = request.dbsession.query(
            atr, not_atr,
            transflectance_diffuse).filter_by(spectrometer_ID=search).all()
        details = {}
