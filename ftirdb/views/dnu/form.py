...
def form(request):
    ...
    if request.POST:
        ...
        thinking = request.POST['upload']

        myfile = request.POST['upload']

        filename = request.POST['upload']
        permanent_store = 'C:/ftirdb/ftirdb/static/files'
        permanent_file = open(
            os.path.join(permanent_store, myfile.filename.lstrip(os.sep)), 'wb')

        shutil.copyfileobj(myfile.file, permanent_file)
        myfile.file.close()
        permanent_file.close()

        #thinking2 = thinking['upload']
        # one = deform.widget.FileUploadWidget.deserialize('upload',form())
        print(thinking)
