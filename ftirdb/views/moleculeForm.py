...
@view_config(route_name='moleculeForm',
             renderer='../templates/moleculeForm.jinja2')
def moleculeForm(request):
    ...
    if 'submit' in request.POST:
        ...
        try:
            ...
            molecule_id = request.dbsession.query(molecule).order_by(
                molecule.molecule_ID.desc()).first()
            ...
            next_url = request.route_url('moleculePage',
...
@view_config(route_name='moleculeForm2',
             renderer='../templates/moleculeForm2.jinja2')
...
@view_config(route_name='moleculePage',
             renderer='../templates/moleculePage.jinja2')
def moleculePage(request):
    ...
    if 'form.submitted' in request.params:
        ...
        searchdb = request.dbsession.query(molecule).filter_by(
            molecule_ID=search).all()
        dic = {}
        for i in searchdb:
            new = i.__dict__
            dic.update(new)
        # search chemical, protein and other table for related details
        searchprotein = request.dbsession.query(
            chemical, protein, other).filter_by(molecule_ID=search).all()
        moldic = {}
