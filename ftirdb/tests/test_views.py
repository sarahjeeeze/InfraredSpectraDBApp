...
class BaseTest(unittest.TestCase):
    ...
    def makePage(self, name, data, creator):
        from ..models import FTIRModel
        return FTIRModel(name=name, data=data, creator=creator)
...
class AddPageTests(BaseTest):
    ...
    def test_it_pageexists(self):
        ...
        pagecount = self.session.query(FTIRModel).filter_by(name='AnotherPage').count()
    ...
    def test_it_submitted(self):
        ...
        request.context = NewPage('AnotherPage')
        self._callFUT(request)
        page = self.session.query(FTIRModel).filter_by(name='AnotherPage').one()
